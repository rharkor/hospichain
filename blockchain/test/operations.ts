...
describe("Operations", function () {
    ...
    describe("Operations", function () {
        it("Should add a operation", async function () {
            ...
            console.log("-->", await praticiens.read.PraticienCount());

            await operations.write.addOperation([newOperation], {
                account: praticien1.account.address,
            })

            await expect(operations.write.addOperation([newOperation], {
                account: manager1.account.address,
            })).to.be.rejectedWith(
                "Caller is not a praticien"
            )

            expect(await operations.read.getOperations([0, 1], {
                account: praticien1.account.address,
            }
            )).to.deep.equal([
                {
